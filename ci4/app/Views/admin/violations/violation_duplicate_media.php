			...
			<div class="header">
				<div>
					<h2 style="display: inline-block;">
						Violations List(Duplicate Media)
					</h2>
				...
				<div>
					<p>List of all duplicate violations, where the photo URL OR the video URL appear in at least one other violation, regardless of violation status. However, the table below only shows violations that are in status New (1), Reviewed (2), Ready to mail (7)</p>
				</div>
			</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		var table = $('#na_datatable').DataTable({
			"processing": true,
			"serverSide": true,
			"ajax": "<?= base_url('admin/violations/datatable_json_duplicate_media') ?>",
			"order": [
				[1, 'asc']
			],
